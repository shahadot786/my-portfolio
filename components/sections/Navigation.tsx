          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`flex flex-col items-center justify-center gap-1 w-full h-full transition-colors ${isActive ? "text-primary" : "text-zinc-500 hover:text-primary"
                  }`}
              ...
              >
                ...
                {isActive && (
                  <span className="absolute bottom-2 w-1.5 h-1.5 rounded-full bg-primary shadow-[0_0_8px_hsl(var(--primary)/0.5)]" />
                )}
              </Link>
            );
          })}
