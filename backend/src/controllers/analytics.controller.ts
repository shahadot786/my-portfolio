...
export const trackVisit = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (type === 'view') {
      ...
      else analytics.referrers.push({ name: referrer, count: 1 });
      ...
      if (language) {
        ...
        if (langIndex > -1) analytics.languages[langIndex].count += 1;
      ...
      if (screen) {
        ...
        if (screenIndex > -1) analytics.screens[screenIndex].count += 1;
